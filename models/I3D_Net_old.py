...
class I3D_Net(object):
    def __init__(self, data_provider, keep_prob, dataset, weight_decay,
        ...
        """
        Class to implement networks base on this paper (Inception-v1 I3D architecture).
    ...
    def Unit3D(self, _input, _out_channels,
        ...
        :param ksize: the filter size of shape [l_size, k_size, k_size, in_channels, out_channels].
    ...
    def Inception_module(self, _input, Inc_params, padding='SAME'):
        """
        Implementation the Inception Module(Inc.) mentioned in the paper.
        :param _input: Inputs to the Inception Inc.
        :return: Outputs from the Inc.
        """
        ...
        with tf.variable_scope('Branch_1'):
            branch_1 = self.Unit3D(_input, Inc_params['Branch_1'][0], Inc_params['Branch_1'][1],
                                   padding, name='conv_1x1x1')
            branch_1 = self.Unit3D(branch_1, Inc_params['Branch_1'][2], Inc_params['Branch_1'][3],
                                   padding, name='conv_3x3x3')
        with tf.variable_scope('Branch_2'):
            branch_2 = self.Unit3D(_input, Inc_params['Branch_2'][0], Inc_params['Branch_2'][1],
                                   padding, name='conv_1x1x1')
            branch_2 = self.Unit3D(branch_2, Inc_params['Branch_2'][2], Inc_params['Branch_2'][3],
                                   padding, name='conv_3x3x3')
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('Conv3d_7x7x7'):
            net = self.Unit3D(self.videos, _out_channels=64, ksize=7, padding='SAME', name='conv_7x7x7',
                              strides=[1, 2, 2, 2, 1])
            net = self.pool(net, ksize=[1, 1, 3, 3, 1], strides=[1, 1, 2, 2, 1], name='Max_pool_1x3x3')
        print('net1:', net.get_shape())  # net1: (?, 4, 56, 56, 64)

        # Convolution 3x3x3
        with tf.variable_scope('Conv3d_3x3x3'):
            net = self.Unit3D(net, _out_channels=192, ksize=3, padding='SAME', name='conv_3x3x3')
            net = self.pool(net, ksize=[1, 1, 3, 3, 1], strides=[1, 1, 2, 2, 1], name='Max_pool_1x3x3')
        print('net2:', net.get_shape())   # net1: (?, 4, 28, 28, 192)

        # first Inception module (Inc_1)
        with tf.variable_scope('Inc_1'):
            net = self.Inception_module(net, self.Inc_params['Inc_1'])
        print('net3:', net.get_shape())   # net3: (?, 4, 28, 28, 256)

        # second Inception module (Inc_2)
        with tf.variable_scope('Inc_2'):
            net = self.Inception_module(net, self.Inc_params['Inc_2'])
            net = self.pool(net, ksize=[1, 3, 3, 3, 1], strides=[1, 2, 2, 2, 1], name='Max_pool_3x3x3')
        print('net4:', net.get_shape())   # net4: (?, 2, 14, 14, 480)

        # third Inception module (Inc_3)
        with tf.variable_scope('Inc_3'):
            net = self.Inception_module(net, self.Inc_params['Inc_3'])
        print('net5:', net.get_shape())  # net5: (?, 2, 14, 14, 512)

        # forth Inception module (Inc_4)
        with tf.variable_scope('Inc_4'):
            net = self.Inception_module(net, self.Inc_params['Inc_4'])
        print('net6:', net.get_shape())   # net6: (?, 2, 14, 14, 512)

        # fifth Inception module (Inc_5)
        with tf.variable_scope('Inc_5'):
            net = self.Inception_module(net, self.Inc_params['Inc_5'])
        print('net7:', net.get_shape())  # net7: (?, 2, 14, 14, 512)

        # sixth Inception module (Inc_6)
        with tf.variable_scope('Inc_6'):
            net = self.Inception_module(net, self.Inc_params['Inc_6'])
        print('net8:', net.get_shape())   # net8: (?, 2, 14, 14, 528)

        # seventh Inception module (Inc_7)
        with tf.variable_scope('Inc_7'):
            net = self.Inception_module(net, self.Inc_params['Inc_7'])
            net = self.pool(net, ksize=[1, 2, 2, 2, 1], strides=[1, 2, 2, 2, 1], name='Max_pool_2x2x2')
        print('net9:', net.get_shape())  # net9: (?, 1, 7, 7, 832)

        # eighth Inception module (Inc_8)
        with tf.variable_scope('Inc_8'):
            net = self.Inception_module(net, self.Inc_params['Inc_8'])
        print('net10:', net.get_shape())   # net10: (?, 1, 7, 7, 832)

        # ninth Inception module (Inc_9)
        with tf.variable_scope('Inc_9'):
            net = self.Inception_module(net, self.Inc_params['Inc_9'])
        ...
        # last Convolution layer 1x1x1
        with tf.variable_scope('Conv3d_1x1x1'):
            logits = self.Unit3D(net, _out_channels=self.n_classes, ksize=1, use_batch_norm=False,
    ...
    def conv3d(self, _input, out_features, kernel_size, name, strides=[1, 1, 1, 1, 1], padding='SAME'):
        ...
        kernel = self.weight_variable_msra([kernel_size, kernel_size, kernel_size, in_features, out_features],
                                           name=name)
    ...
    def calc_motempl(self, last_frame, curr_frame, motion_history):
    ...
    # def calc_rgb_output(self, rgb_data, input_data):
    #     """
    #     Calculate the output of the rgb trained model.
    #     :param rgb_data: ‘list’, the rgb input data.
    #     :param input_data: ‘list’, the data of feedict.
    #     :return:
    #         str action: the type of action.
    #         float prob: the prob of the action.
    #         result: list, the output of the rgb model.
    #     """
        ...
        while camera.isOpened() and camera2.isOpened():
            try:
                ...
                temp_frame2 = cv2.resize(frame2, (224, 224),
                                         interpolation=cv2.INTER_CUBIC)
                # if i % 8 == 0:
                #     cv2.imwrite('./nofights/%s.jpg' % i, frame)
                # i += 1
                # fps = 16
                # fourcc = cv2.VideoWriter_fourcc(*'MJPG')
                # videoWriter = cv2.VideoWriter('./nofight.avi', fourcc, fps, (224, 224))
                # videoWriter.write(temp_frame)
                ...
                if len(flow_data) == model_params['sequence_length']:
                    # input_data.append(flow_data)
                    # data1 = np.array(input_data).astype(np.float32)
                    # start_time = time.time()
                    # feed_dict = {self.videos: data1, self.is_training: None}
                    # fetches = [self.prediction]
                    # result = self.sess.run(fetches, feed_dict=feed_dict)
                    # duration = time.time() - start_time
                    # print('result_video1:', result)
                    # print('forward_duration:%s' % duration)
                    # index = np.argmax(result)
                    # print(index)
                    # type_id = str(index)
                ...
                if len(flow_data2) == model_params['sequence_length']:
                    # input_data2.append(flow_data2)
                    # data2 = np.array(input_data2).astype(np.float32)
                    # start_time = time.time()
                    # feed_dict = {self.videos: data2, self.is_training: None}
                    # fetches = [self.prediction]
                    # result2 = self.sess.run(fetches, feed_dict=feed_dict)
                    # duration = time.time() - start_time
                    # print('result_video2:', result2)
                    # print('forward_duration:%s' % duration)
                    # index2 = np.argmax(result2)
                    # print(index2)
                    # type_id2 = str(index2)
                    # print('type_id:', type_id2)
                    # action2 = action_type[type_id2]
                    # prob2 = result2[0][0][index2]
    ...
    def evaluate_rgb_model(self, video_path1, video_path2, model_params):
        ...
        while camera.isOpened() and camera2.isOpened():
            try:
                ...
                img_data.append(temp_frame)
                img_data2.append(temp_frame2)
                # print('read_frame_duration:', duration)     # duration < 0.01

                # camera1
                if len(img_data) == model_params['sequence_length']:
                    # input_data1.append(img_data)
                    # data1 = np.array(input_data1).astype(np.float32)
                    # # print("data1.shape:", data1.shape)
                    # # print('data1:', data1.dtype)   # float32
                    # start_time = time.time()
                    # feed_dict = {self.videos: data1, self.is_training: None}
                    # fetches = [self.prediction]
                    # result = self.sess.run(fetches, feed_dict=feed_dict)
                ...
                if len(img_data2) == model_params['sequence_length']:
                    print('camera2!!!!!!')
                    # input_data2.append(img_data2)
                    # data2 = np.array(input_data2).astype(np.float32)
                    # # print(data2.shape)    # shape2: (1, 10, 128, 128, 3)
                    # start_time = time.time()
                    # feed_dict = {self.videos: data2, self.is_training: False}
                    # fetches = [self.prediction]
                    # result2 = self.sess.run(fetches, feed_dict=feed_dict)
                    # duration = time.time() - start_time
                    # print('result_video2: ', result)
                    # # print('forward_duration:%s' % duration)
                    # index2 = np.argmax(result)
                    # print(index2)
                    # type_id2 = str(index2)
                    # print('type_id:', type_id2)
                    # action = action_type[type_id2]
                    # prob2 = result2[0][0][index2]
