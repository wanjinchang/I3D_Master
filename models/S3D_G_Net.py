...
class S3D_G_Net(object):
    def __init__(self, data_provider, keep_prob, dataset, weight_decay,
        ...
        self._define_inputs()
        # build the S3D model graph
        # self._build_graph()
        # build the S3D-G model graph
        self._build_s3d_g_graph()
    ...
    def Sep_Conv(self, _input, _out_channels, ksize, padding, name, strides=[1, 1, 1, 1, 1],
        ...
        with tf.variable_scope('Spa_Conv'):
        ...
        with tf.variable_scope('Tem_Conv'):
            net = self.conv3d(net, _out_channels, [ksize[0], 1, 1], name[1], [1, strides[1], 1, 1, 1], padding)
            # Spatial-Temporal feature gating
            # print('net1:', net.get_shape())
            with tf.variable_scope('feature_gating'):
                net = self.feature_gating(net, name=name[1])
        if use_batch_norm:
            net = self.batch_norm(net)
        if activation_fn:
            with tf.name_scope('ReLU'):
    ...
    def Sep_Inc(self, _input, Inc_params, padding='SAME'):
        ...
        with tf.variable_scope('Branch_0'):
            ...
            # Spatial-Temporal feature gating
            with tf.variable_scope('feature_gating'):
                branch_0 = self.feature_gating(branch_0, name='conv_1x1x1')
        ...
        with tf.variable_scope('Branch_3'):
            ...
            # Spatial-Temporal feature gating
            with tf.variable_scope('feature_gating'):
                branch_3 = self.feature_gating(branch_3, name='conv_1x1x1')
    ...
    def feature_gating(self, _input, name):
        # average pooling on _input: NxDxHxWxC --> Nx1x1x1xC
        # print('input_shape:', _input.get_shape())
        _batch, _depth, _height, _width, _channels = _input.get_shape().as_list()
        ksize = [1, _depth, _height, _width, 1]
        strides = [1, 1, 1, 1, 1]
        pooling_vector = self.pool(_input, ksize, strides, name='feature_gating', _type='avg', padding='VALID')
        # pooling_vector.shape [?, 1, 1, 1, _channels] -- > [?, 64]
        pooling_vector = tf.squeeze(pooling_vector, [1, 2, 3], name='squeeze')
        gating_weights = self.weight_variable_normal(shape=[_channels, _channels], name=name,
                                                     stddev=1 / math.sqrt(_channels))
        # input_vector = tf.reshape(pooling_vector, [-1, _depth * _height * _width, -1])
        gates = tf.matmul(pooling_vector, gating_weights)
        adative_weights = tf.sigmoid(gates)
        # adative_weights.shape = [_batch_size, _channels], make it to be shape of [_batch_size, 1, 1, 1, _channels]
        adative_weights = tf.expand_dims(adative_weights, 1)
        adative_weights = tf.expand_dims(adative_weights, 2)
        adative_weights = tf.expand_dims(adative_weights, 3)
        # adative_weights.shape = [_batch_size, 1, 1, 1, _channels], make it to be
        # shape of [_batch_size, _depth, _height, _width, _channels]
        adative_weights = tf.tile(adative_weights, [1, _depth, _height, _width, 1])
        activation = tf.multiply(adative_weights, _input)
        return activation
    ...
    def weight_variable_normal(self, shape, name, stddev):
        return tf.get_variable(
            name, shape=shape,
